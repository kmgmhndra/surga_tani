...
// Class helper untuk menampung controller di halaman edit
class PerkebunanEditData {
  String nama;
...
class DetailPerkebunanScreen extends StatefulWidget {
  final PencatatanPerkebunan pencatatan;
  final int pencatatanKey;

  const DetailPerkebunanScreen({
    super.key,
  ...
  @override
  State<DetailPerkebunanScreen> createState() => _DetailPerkebunanScreenState();
}

class _DetailPerkebunanScreenState extends State<DetailPerkebunanScreen> {
  ...
  late List<PerkebunanEditData> _komoditiEditList;
  List<dynamic> _images = [];
  final ImagePicker _picker = ImagePicker();

  // Constants
  static const Color _primaryColor = Color(0xFF689F38);
  ...
  void initState() {
    ...
    _komoditiEditList = widget.pencatatan.daftarKomoditi.map((komoditi) {
      return PerkebunanEditData(
        nama: komoditi.nama,
        hargaController: TextEditingController(
          text: komoditi.hargaPerKg == 0.0
              ? ''
              : komoditi.hargaPerKg?.toStringAsFixed(0),
        ),
  ...
  Future<void> _pickImages() async {
    try {
      ...
      if (mounted) {
        showTopSnackBar(
          Overlay.of(context),
          const CustomSnackBar.error(message: "Gagal memilih gambar."),
        );
      }
    }
  }
  ...
  void _updateData() async {
    ...
    final updatedListKomoditi = _komoditiEditList.map((editData) {
      return KomoditiSederhana()
        ..nama = editData.nama
        ..hargaPerKg = double.tryParse(editData.hargaController.text) ?? 0.0
        ..keterangan = editData.keteranganController.text;
    }).toList();

    final updatedPencatatan = PencatatanPerkebunan()
    ...
    if (mounted) {
      ...
      showTopSnackBar(
        ...
        const CustomSnackBar.success(
          message: "Data Perkebunan berhasil diperbarui!",
        ),
      );
