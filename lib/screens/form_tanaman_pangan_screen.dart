...
import 'package:surgatani/providers/form_provider.dart';
import 'package:surgatani/widgets/gradient_app_bar.dart';

class FormTanamanPanganScreen extends StatefulWidget {
  const FormTanamanPanganScreen({super.key});

  @override
  State<FormTanamanPanganScreen> createState() =>
      _FormTanamanPanganScreenState();
}

class _FormTanamanPanganScreenState extends State<FormTanamanPanganScreen> {
  ...
  void _showOverlayLoading({String message = 'Memproses data...'}) {
    _hideOverlayLoading(); // Pastikan tidak ada overlay sebelumnya
  ...
  void _showAddCommodityDialog(FormProvider provider) {
                ...
                child: ElevatedButton(
                  onPressed: () {
                    if (newCommodityController.text.trim().isNotEmpty) {
                      provider
                          .tambahKomoditi(newCommodityController.text.trim());
  ...
  Future<void> _saveData(FormProvider provider) async {
    // Unfocus keyboard
    ...
    try {
      ...
      _showOverlayLoading(message: 'Menyimpan data...');

      // Small delay to ensure overlay is shown
      await Future.delayed(const Duration(milliseconds: 300));

      // Prepare commodity data
      final List<Komoditi> daftarKomoditi = provider.items.map((item) {
        return Komoditi()
          ..nama = item.nama
          ..hargaEceran = double.tryParse(item.hargaEceranController.text) ?? 0
          ..hargaGrosir = double.tryParse(item.hargaGrosirController.text) ?? 0
      ...
      // Create new record
      final newPencatatan = PencatatanTanamanPangan()
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildSectionHeader("Daftar Harga Komoditi",
                        Icons.monetization_on_outlined),
                    const SizedBox(height: 20),
                    _buildCommoditySection(provider),
                    const SizedBox(height: 32),
                    _buildSectionHeader(
  ...
  Widget _buildCommoditySection(FormProvider provider) {
    return Column(
      children: [
        if (_isLoadingCommodities)
          ...
          ListView.separated(
            ...
            itemBuilder: (context, index) =>
                _buildCommodityCard(provider, index),
          ),
        const SizedBox(height: 16),
        _buildAddCommodityButton(provider),
      ],
    );
  }

  Widget _buildCommodityCard(FormProvider provider, int index) {
              ...
              children: [
                ...
                if (!item.isDefault)
                  IconButton(
                    ...
                    onPressed: () => provider.hapusKomoditi(index),
  ...
  Widget _buildAddCommodityButton(FormProvider provider) {
    return Container(
      ...
      child: InkWell(
        onTap: () => _showAddCommodityDialog(provider),
        borderRadius: BorderRadius.circular(12),
        child: Padding(
  ...
  Widget _buildSaveButton(FormProvider provider) {
    return Container(
      decoration: BoxDecoration(
