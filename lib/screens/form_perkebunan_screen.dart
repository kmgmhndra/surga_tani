...
import 'package:surgatani/models/pencatatan_hortikultura_model.dart'; // Menggunakan KomoditiSederhana
import 'package:image_picker/image_picker.dart';
import 'package:path_provider/path_provider.dart';
import 'package:path/path.dart' as path;
import 'package:surgatani/widgets/gradient_app_bar.dart'; // Import widget kustom
import 'package:top_snackbar_flutter/custom_snack_bar.dart';
import 'package:top_snackbar_flutter/top_snack_bar.dart';

// Class helper untuk state management form ini
class PerkebunanFormData {
  final String nama;
  final bool isDefault;
  final TextEditingController hargaController = TextEditingController();
  final TextEditingController keteranganController = TextEditingController();
  PerkebunanFormData({required this.nama, this.isDefault = false});
}

class FormPerkebunanScreen extends StatefulWidget {
  const FormPerkebunanScreen({super.key});

  @override
  State<FormPerkebunanScreen> createState() => _FormPerkebunanScreenState();
}

class _FormPerkebunanScreenState extends State<FormPerkebunanScreen> {
  ...
  late List<PerkebunanFormData> _komoditiItems;

  // Constants
  static const Color _primaryColor =
      Color(0xFF689F38); // Warna tema Perkebunan (Hijau)
  ...
  List<PerkebunanFormData> _getDefaultCommodities() {
    const defaultNames = [
      'Kopi Biji Kering per Kg',
      'Lada Biji Kering per Kg',
      'Kakao non fermentasi per Kg',
      'Karet Lump per Kg',
      'Kelapa dalam per butir',
      'Tebu per Kg',
      'Cengkeh kering per Kg',
    ];
    return defaultNames
        .map((nama) => PerkebunanFormData(nama: nama, isDefault: true))
        .toList();
  }
  ...
  Future<void> _saveData() async {
    ...
    try {
      ...
      final List<KomoditiSederhana> daftarKomoditi = _komoditiItems.map((item) {
        return KomoditiSederhana()
          ..nama = item.nama
          ..hargaPerKg = double.tryParse(item.hargaController.text) ?? 0.0
      ...
      final newPencatatan = PencatatanPerkebunan()
      ...
      final box = Hive.box('perkebunan_box');
      ...
      if (mounted) {
        ...
        showTopSnackBar(
          ...
          const CustomSnackBar.success(
              message: "Data Perkebunan berhasil disimpan!"),
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: GradientAppBar(
        ...
        icon: Icons.park,
        gradientColors: [
          const Color(0xFF7CB342), // Lighter green
          _primaryColor, // Main green
    ...
    return Row(children: [
      ...
      Text(
        ...
        style: const TextStyle(
          ...
          color: Color(0xFF33691E), // Warna teks lebih gelap
        ),
      ),
    ]);
  }
  ...
  Widget _buildGeneralInfoSection() {
    return Column(children: [
      ...
      _buildTextField(
        ...
        label: 'Lokasi (Contoh: Pasar Induk)',
